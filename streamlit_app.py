...
def color_replace(image, mask, alpha=0.5):
    # Convert to HSV float32 for blending
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV).astype(np.float32)
    # Random target hue, saturation, value
...
def apply_individual_colors(image, sam_masks, alpha=0.5):
    out = image.copy()
    for m in sam_masks:
...
if uploaded:
    ...
    if seg_color or env_swap_opt:
        ...
        if seg_color:
            img = apply_individual_colors(img, sam_masks, color_alpha)
