...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Lerobot dataset augmentation: lighting, noise, seg/color swap, env swap with progress tracking"
    )
    ...
    parser.add_argument("--light",     action="store_true", help="Apply brightness/contrast jitter")
    parser.add_argument("--noise",     action="store_true", help="Apply Gaussian noise to vector data")
    parser.add_argument("--seg-color", action="store_true", help="Segment & random-color robot")
...
def main():
    ...
    ds = LeRobotDataset(repo_id=args.repo, root=repo_cache, local_files_only=False, download_videos=True)
    ...
    img_ops = []
    if args.light:    img_ops.append(RandomBrightnessContrast(p=1))
    ...
    if out_cache.exists():  shutil.rmtree(out_cache)
    new_ds = LeRobotDataset.create(repo_id=out_repo, fps=ds_meta.fps, features=ds_meta.features, root=out_cache)
    cam_key = ds_meta.camera_keys[0]

    # Episode loop with progress bar
    for ep in tqdm(range(ds_meta.total_episodes), desc="Episodes", unit="ep"):
        ...
        for i, row in enumerate(tqdm(hf_ds, desc=f"Frames ep {ep+1}", total=total_frames, unit="fr")):
            ...
            if args.seg_color or args.env_swap:
                ...
                if args.seg_color: img = color_replace(img, mask)
                if args.env_swap and args.bg_dir: img = env_swap(img, mask, args.bg_dir)

            img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            frame_data = {cam_key: img_rgb}
            for vk in vector_keys:
                vec = np.array(row[vk])
                if args.noise: vec = vec + np.random.randn(*vec.shape) * 0.01
                frame_data[vk] = vec

            new_ds.add_frame(frame_data)

        cap.release()
        new_ds.save_episode(task="teleop", encode_videos=True)

    # Push to HF with final message
